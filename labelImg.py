...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def file_item_double_clicked(self, item=None):
        self.cur_img_idx = self.img_paths.index(ustr(item.text()))
    ...
    def load_file(self, file_path=None):
        ...
        if unicode_file_path and self.file_list_widget.count() > 0:
            if unicode_file_path in self.img_paths:
                index = self.img_paths.index(unicode_file_path)
    ...
    def copy_previous_bounding_boxes(self):
        current_index = self.img_paths.index(self.file_path)
        if current_index - 1 >= 0:
            prev_file_path = self.img_paths[current_index - 1]
